...
def seed_cards_data(apps, schema_editor):
...
def remove_cards_data(apps, schema_editor):
    CreditCard = apps.get_model('cards', 'CreditCard')
    card_names = [c["card_name"] for c in CARDS_DATA]
    CreditCard.objects.filter(card_name__in=card_names).delete()

class Migration(migrations.Migration):

    dependencies = [
        ('cards', '0003_remove_creditcard_bank_alter_creditcard_options_and_more'),
    ]

    operations = [
        migrations.RunPython(seed_cards_data, remove_cards_data),
    ]
